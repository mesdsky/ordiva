  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#F5F2E8] text-[#173C34]">
      <section className="relative overflow-hidden border-b border-[#DDE6D7] bg-[linear-gradient(180deg,#DDE8D8_0%,#E5ECDF_28%,#EEF0E7_58%,#F5F2E8_100%)]">
        ...
        <div className="relative mx-auto max-w-6xl px-6 pb-16 pt-8 md:px-12 md:pb-24 md:pt-10">
      ...
      <section className="mx-auto max-w-6xl px-6 py-12 md:px-12 md:py-16">
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {plans.map((plan) => {
            ...
            return (
              <article
                key={plan.id}
                className={`relative flex flex-col rounded-[2rem] border p-6 transition-[transform,box-shadow,background-color] duration-200 md:p-7 ${
                  isSelected
              ...
              >
                ...
                <p className="mt-6 text-3xl font-bold tracking-[-0.04em] text-[#173C34]">
                  {formatIDR(plan.price)}
                </p>

                <p className="mt-1 text-xs font-medium text-[#7B9685]">
                  {plan.period}
                </p>
                ...
                {plan.months && (
                  <div className="mt-4 rounded-xl bg-[#E8EEDB]/65 px-3 py-2">
                    ...
                    <p className="mt-1 text-[10px] font-medium text-[#5F7168]">
                      ...
                      {" "}
                      ({Math.round(
                        (1 - plan.price / (29000 * plan.months)) * 100
                      )}% vs monthly)
                    </p>
                  </div>
                )}

                <p className="mt-5 min-h-12 text-sm leading-6 text-[#5F7168]">
                  {plan.description}
                </p>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.8fr]">
          <section className="rounded-[2rem] border border-[#DDE6D7] bg-white/55 p-7 shadow-[0_12px_35px_rgba(23,60,52,0.045)] md:p-9">
            <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#7B9685]">
              What you unlock
            </p>

            <h2 className="mt-3 text-2xl font-semibold tracking-[-0.035em] text-[#173C34] md:text-3xl">
            ...
            <div className="mt-7 grid gap-4 sm:grid-cols-2">
              {benefits.map((benefit) => (
                <div
                  key={benefit}
                  className="flex gap-3 rounded-2xl border border-[#DDE6D7] bg-[#F9F8F2]/75 p-4"
                >
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#E8EEDB] text-sm font-bold text-[#214F43]">
                    ✓
                  </span>
          ...
          <section className="rounded-[2rem] bg-[#214F43] p-7 text-white shadow-[0_25px_70px_rgba(33,79,67,0.12)] md:p-9">
            ...
            <button
              type="button"
              disabled
              className="mt-7 w-full cursor-not-allowed rounded-2xl bg-white/20 px-5 py-3.5 text-sm font-semibold text-white/60"
            >
              Checkout coming soon
            </button>

            <p className="mt-4 text-center text-xs leading-5 text-white/40">
              Secure checkout will be available when Ordiva Premium launches.
            </p>
          </section>
        </div>
      </section>
