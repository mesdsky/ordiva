...
function getGivenNames(user: { email?: string; user_metadata?: Record<string, unknown> }) {
  ...
  const value = String(metadataName || fallbackName).trim();
  return value.slice(0, 100) || "Ordiva customer";
}
...
export async function POST(request: Request) {
  try {
    ...
    const session = await xenditRequest<XenditSessionResponse>(
      "/sessions",
      {
        method: "POST",
    ...
    const sessionId =
      session.payment_session_id ?? session.id ?? null;
